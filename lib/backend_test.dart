...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _sendCode() => _guard(() async {
    ...
    _append(
      '📱 인증번호 발송됨 (유효 ${data['expiresInSec']}초). '
  ...
  Future<void> _register() => _guard(() async {
    ...
    final opts = await _post('/auth/passkey/registration/options', {
      'displayName': '보호자',
    ...
    final reg = await _passkey.register(
      RegisterRequestType(
    ...
    _append(
      '🎉 가입 완료! protectorId=${data['protectorId']} '
      'accessToken 발급됨',
    );
    setState(() {});
  });
  ...
  Future<void> _login() => _guard(() async {
    ...
    final auth = await _passkey.authenticate(
              ...
              Wrap(
                ...
                children: [
                  FilledButton(
                    ...
                    child: const Text('1. 인증확인'),
                  ),
                  FilledButton(
                    onPressed: _busy || !canRegister ? null : _register,
                    child: const Text('2. 패스키 등록'),
                  ),
                  FilledButton(
                    onPressed: _busy ? null : _login,
                    child: const Text('3. 로그인'),
                  ),
                  FilledButton(
